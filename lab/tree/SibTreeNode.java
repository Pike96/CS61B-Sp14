...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (isValidNode()) {
      ...
      if (this.child(c).isValidNode()) {
        if (c == 1) {
          newKid.nextSibling = firstChild;
          firstChild = newKid;
        } else {
          newKid.nextSibling = ((SibTreeNode) this.child(c-1)).nextSibling;
          ((SibTreeNode) this.child(c-1)).nextSibling = newKid;
        }
      } else {
        SibTreeNode kid = firstChild;
        while (kid.nextSibling().isValidNode()) {
          kid = kid.nextSibling;
        }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (firstChild == null) {
      if (isValidNode()) {
        ...
        if (this == ((SibTreeNode) this.parent()).firstChild) {
        	((SibTreeNode) this.parent()).firstChild = nextSibling;
          valid = false;
          myTree.size--;
          return;
        }
        SibTreeNode kid = ((SibTreeNode) this.parent()).firstChild;
