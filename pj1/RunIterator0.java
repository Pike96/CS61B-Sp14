...
public class RunIterator implements Iterator {
  ...
  public int[] next() {
    ...
    int[] para = new int[4];
    para[0] = node.item.intensity[0];
    para[1] = node.item.intensity[1];
    para[2] = node.item.intensity[2];
    para[3] = node.item.length;
    return para;
  }
